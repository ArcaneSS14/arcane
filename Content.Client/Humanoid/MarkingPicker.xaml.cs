...
public sealed partial class MarkingPicker : Control
{
    ...
    private IReadOnlyDictionary<string, MarkingPrototype> GetMarkings(MarkingCategories category)
    {
        ...
        foreach (var (id, marking) in _markingManager.MarkingsByCategory(category))
        {
            if (IgnoreSpecies)
            {
                if (marking.SexRestriction != null && marking.SexRestriction != _currentSex)
                {
                    continue;
                }

                result.Add(id, marking);
                continue;
            }

            var speciesProto = _prototypeManager.Index<SpeciesPrototype>(_currentSpecies);
            ...
            if (marking.SpeciesRestrictions != null && !marking.SpeciesRestrictions.Contains(_currentSpecies))
            {
                continue;
            }
    ...
    private bool CanUseMarking(MarkingPrototype marking)
    {
        return MarkingWhitelistManager.IsMarkingAllowed(marking, _playerManager.LocalSession);
    }

    private void ShowRestrictedMarkingPopup()
    {
        _userInterfaceManager.Popup(Loc.GetString("markings-picker-user-only"));
    }
    ...
    private void OnUnusedMarkingSelected(ItemList.ItemListSelectedEventArgs item)
    {
        var selected = CMarkingsUnused[item.ItemIndex];
        var prototype = (MarkingPrototype) selected.Metadata!;

        if (!CanUseMarking(prototype))
        {
            selected.Selected = false;
            _selectedUnusedMarking = null;
            ShowRestrictedMarkingPopup();
            return;
        }

        _selectedUnusedMarking = selected;
    }
    ...
    private void OnUsedMarkingSelected(ItemList.ItemListSelectedEventArgs item)
    {
        ...
        if (!CanUseMarking(prototype))
        {
            _selectedMarking.Selected = false;
            _selectedMarking = null;
            ShowRestrictedMarkingPopup();
            return;
        }
    ...
    private void MarkingAdd()
    {
        ...
        if (!_markingManager.CanBeApplied(_currentSpecies, _currentSex, marking, _prototypeManager, _playerManager.LocalSession))
        {
            ShowRestrictedMarkingPopup();
            return;
        }

...
public sealed partial class SingleMarkingPicker : BoxContainer
{
    ...
    public void UpdateData(List<Marking> markings, string species, int totalPoints)
    {
        ...
        if (session != null)
        {
            var filtered = new MarkingSet(_markings, _markingManager);
            filtered.EnsureWhitelisted(session, _markingManager);
            _markings = filtered.GetForwardEnumerator().ToList();
        }

        _markingPrototypeCache = _markingManager.MarkingsByCategoryAndSpecies(Category, _species, session);
    ...
    public void PopulateList(string filter)
    {
        ...
        _markingPrototypeCache ??= GetMarkings(Category, _species);
    ...
    private void SelectMarking(ItemList.ItemListSelectedEventArgs args)
    {
        ...
        if (!MarkingWhitelistManager.IsMarkingAllowed(proto, _playerManager.LocalSession))
        {
            _ignoreItemSelected = true;
            MarkingList[args.ItemIndex].Selected = false;
            _ignoreItemSelected = false;
    ...
    private IReadOnlyDictionary<string, MarkingPrototype> GetMarkings(MarkingCategories category, string species)
    {
        var result = new Dictionary<string, MarkingPrototype>();
        var speciesProto = _markingManager == null ? null : default(SpeciesPrototype);
        var currentSpecies = _prototypeManager.Index<SpeciesPrototype>(species);
        var markingPoints = _prototypeManager.Index(currentSpecies.MarkingPoints);

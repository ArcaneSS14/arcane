...
alert-level-amber-locked = Red alert is locked. It must be unlocked by command authorization or a station-ending threat before it can be activated.
alert-level-amber-unlocked = Red alert unlocked. It can now be activated from any communications console.
alert-level-amber-no-id = No ID card detected. Hold or wear an ID to swipe.
alert-level-amber-needs-command = Unlocking red alert must be initiated with a Captain or Head of Security ID.
alert-level-amber-first-swipe = First ID accepted. Swipe a second, different command ID to unlock the red alert.
...
alert-level-amber-verb-text = Unlock Red Alert
alert-level-amber-verb-message = Swipe your ID to unlock red alert so it can be activated. Requires a Captain or Head of Security ID plus a second, different command ID.
alert-level-amber-authorized-initiated-announcement = Red alert unlock initiated by {$name}.
alert-level-amber-authorized-announcement = Red alert has been unlocked by {$name}.
alert-level-amber-unknown-name = Unknown
alert-level-amber-unlocked-announcement = Red alert has been unlocked and can now be activated from any communications console.

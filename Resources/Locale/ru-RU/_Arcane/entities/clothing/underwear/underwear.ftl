ent-ClothingUnderwearDriada = трусы дриады
    .desc = Покровительница деревьев.
ent-ClothingUnderwearEroticPurple = фиолетовые эротические трусы
    .desc = Тонкие трусики, едва прикрывающие гениталии.
ent-ClothingUnderwearMedusa = медузьи трусы
    .desc = Желейные трусы.
ent-ClothingUnderwearLeafs = трусы из листьев
    .desc = 100% натуральные.
ent-ClothingUnderwearPinkFancy = модные розовые трусы
    .desc = Розовые, как Барби.
ent-ClothingUnderwearRed = красные трусы
    .desc = Красные трусы.
ent-ClothingUnderwearAx = трусы Акса
    .desc = Уга буга.

ent-ClothingUnderwearBeerPants = пивные трусы
    .desc = Пивные трусы.

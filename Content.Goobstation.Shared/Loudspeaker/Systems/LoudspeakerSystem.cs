...
public sealed class LoudSpeakerSystem : EntitySystem
{
    ...
    {
        var state = ent.Comp.IsActive ? "loudspeaker-state-on" : "loudspeaker-state-off"; // Arcane-Edit-Locale

        var message = ent.Comp.CanToggle
            ? Loc.GetString("loudspeaker-examine-toggleable", ("state", Loc.GetString(state))) // Arcane-Edit-Locale
    ...
    {
        var state = !loudspeaker.Comp.IsActive ? "loudspeaker-state-on" : "loudspeaker-state-off"; // Arcane-Edit-Locale

        _audio.PlayPredicted(loudspeaker.Comp.ToggleSound, user, user);
        _popup.PopupClient(Loc.GetString("loudspeaker-toggle-popup", ("state", Loc.GetString(state))), user, user); // Arcane-Edit-Locale
    }

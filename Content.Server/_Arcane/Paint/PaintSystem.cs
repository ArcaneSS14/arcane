...
namespace Content.Server._Arcane.Paint;
...
public sealed class PaintSystem : SharedPaintSystem
{
    ...
    private void OnPaintVerb(EntityUid uid, PaintComponent component, GetVerbsEvent<UtilityVerb> args)
    {
        ...
        var verb = new UtilityVerb()
        {
            ...
            Text = paintText,
            Icon = new SpriteSpecifier.Texture(new("/Textures/_Arcane/Interface/VerbIcons/paint.svg.192dpi.png"))
        };
        args.Verbs.Add(verb);
    }
    ...
    public void Paint(Entity<PaintComponent> entity, EntityUid target, EntityUid user)
    {
        ...
        if (HasComp<ArcanePaintedComponent>(target) || HasComp<RandomSpriteComponent>(target))
        {
            _popup.PopupEntity(Loc.GetString("paint-failure-painted", ("target", target)), user, user, PopupType.Medium);
            return;
        }

        if (!_whitelist.CheckBoth(target, entity.Comp.Blacklist, entity.Comp.Whitelist)
        ...
        {
            EnsureComp<ArcanePaintedComponent>(target, out var paint);
            ...
            if (HasComp<InventoryComponent>(target)
                ...
                foreach (var slot in slotDefinitions)
                {
                    if (!_inventory.TryGetSlotEntity(target, slot.Name, out var slotEnt)
                        ...
                        || !_whitelist.CheckBoth(slotEnt.Value, entity.Comp.Blacklist, entity.Comp.Whitelist)
                        || HasComp<RandomSpriteComponent>(slotEnt.Value)
                        || HasComp<HumanoidAppearanceComponent>(slotEnt.Value))
                        continue;

                    EnsureComp<ArcanePaintedComponent>(slotEnt.Value, out var slotToPaint);
    ...
    {
        if (!_whitelist.CheckBoth(target, blacklist, whitelist))
            return;
        ...
        if (HasComp<InventoryComponent>(target)
            ...
            {
                if (!_inventory.TryGetSlotEntity(target, slot.Name, out var slotEnt)
                    || !_whitelist.CheckBoth(slotEnt.Value, blacklist, whitelist))
                    continue;

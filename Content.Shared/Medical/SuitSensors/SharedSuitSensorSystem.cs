...
public abstract class SharedSuitSensorSystem : EntitySystem
{
    ...
    public SuitSensorStatus? GetSensorState(Entity<SuitSensorComponent?, TransformComponent?> ent)
    {
        ...
        var status = new SuitSensorStatus(GetNetEntity(sensor.User.Value), GetNetEntity(ent.Owner), userName, userJob, userJobIcon, userJobDepartments, sensor.Mode); // Orion
        switch (sensor.Mode)
        {
            case SuitSensorMode.SensorBinary:
    ...
    public NetworkPayload SuitSensorToPacket(SuitSensorStatus status)
    {
        ...
        payload.Add(SuitSensorConstants.NET_SUIT_SENSOR_MODE, status.Mode); // Orion
    ...
    public SuitSensorStatus? PacketToSuitSensor(NetworkPayload payload)
    {
        ...
        if (!payload.TryGetValue(SuitSensorConstants.NET_SUIT_SENSOR_MODE, out SuitSensorMode sensorMode)) return null; // Orion
        ...
        var status = new SuitSensorStatus(ownerUid, suitSensorUid, name, job, jobIcon, jobDepartments, sensorMode) // Orion
        {
